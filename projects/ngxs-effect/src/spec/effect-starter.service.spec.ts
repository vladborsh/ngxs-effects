...
class StateStub {
  @Action(ActionA)
  a(context: StateContext<Record<string, any>>, {payload}: ActionA) {
    context.setState({ ...context.getState(), [payload.id]: payload });
  }

  @Action(ActionB)
  b(context: StateContext<Record<string, any>>, {payload}: ActionA) {
    context.setState({ ...context.getState(), [payload.id]: undefined });
  }
}
  ...
  describe('should start several effects', () => {
    ...
    it('should not interrupt action processing', fakeAsync(() => {
      ...
      store.select(state => state)
        .subscribe(state => selectResult = state);
      tick();
      expect(selectResult).toEqual({ state: { 'test-id': undefined } });
    }));
  });
